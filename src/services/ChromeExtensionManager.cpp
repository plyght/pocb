...
- (void)webExtensionController:(WKWebExtensionController *)controller presentPopupForAction:(WKWebExtensionAction *)action forExtensionContext:(WKWebExtensionContext *)context completionHandler:(void (^)(NSError *))completionHandler {
    (void)controller;
    (void)context;
    if (!action.presentsPopup || !action.popupPopover) {
        completionHandler([NSError errorWithDomain:@"pocb.extensions" code:4 userInfo:@{NSLocalizedDescriptionKey:@"No popup is available"}]);
        return;
    }
    static PocbExtensionPopupDelegate *popupDelegate = nil;
    if (!popupDelegate) popupDelegate = [PocbExtensionPopupDelegate new];
    action.popupWebView.UIDelegate = popupDelegate;
    NSView *anchor = g_popupAnchor;
    if (!anchor && g_browserWindow) {
        g_browserWindow->winId();
        anchor = (__bridge NSView *)reinterpret_cast<void *>(g_browserWindow->winId());
    }
    if (!anchor) {
        completionHandler([NSError errorWithDomain:@"pocb.extensions" code:5 userInfo:@{NSLocalizedDescriptionKey:@"No browser view is available"}]);
        return;
    }
    [action.popupPopover showRelativeToRect:anchor.bounds ofView:anchor preferredEdge:NSMinYEdge];
    NSURL *activeURL = nil;
    if (g_browserWindow && g_browserWindow->extensionCurrentView()) {
        QUrl qurl = g_browserWindow->extensionCurrentView()->url();
        if (qurl.isValid()) activeURL = [NSURL URLWithString:qurl.toString().toNSString()];
    }
    if (activeURL && ([activeURL.scheme isEqualToString:@"http"] || [activeURL.scheme isEqualToString:@"https"])) {
        WKWebView *activeWebView = g_browserWindow && g_browserWindow->extensionCurrentView() ? (__bridge WKWebView *)g_browserWindow->extensionCurrentView()->nativeWebView() : nil;
        QJsonArray blockedRules;
        for (const QString &path : ChromeExtensionManager::configuredPaths()) {
            const QJsonArray rules = contentRuleListPayloadForExtension(path, 30000);
            for (const QJsonValue &value : rules) {
                const QJsonObject rule = value.toObject();
                if (rule.value(QStringLiteral("action")).toObject().value(QStringLiteral("type")).toString() == QStringLiteral("block")) {
                    const QString filter = rule.value(QStringLiteral("trigger")).toObject().value(QStringLiteral("url-filter")).toString();
                    if (!filter.isEmpty()) blockedRules.append(filter);
                }
            }
        }
        NSString *blockedRulesJSON = QString::fromUtf8(QJsonDocument(blockedRules).toJson(QJsonDocument::Compact)).toNSString();
        NSString *collector = @"(function(){try{var urls=[];function add(u){try{u=String(u||'');if(/^https?:/.test(u)&&urls.indexOf(u)<0)urls.push(u);}catch(e){}}try{performance.getEntriesByType('resource').forEach(function(e){add(e.name);});}catch(e){}try{document.querySelectorAll('[src],[href]').forEach(function(el){add(el.currentSrc||el.src||el.href);});}catch(e){}return urls.slice(0,1000);}catch(e){return[];}})()";
        [activeWebView evaluateJavaScript:collector completionHandler:^(id result, NSError *pageError) {
            (void)pageError;
            NSMutableArray<NSString *> *urls = [NSMutableArray array];
            if ([result isKindOfClass:[NSArray class]]) {
                for (id value in (NSArray *)result) if ([value isKindOfClass:[NSString class]]) [urls addObject:value];
            }
            NSData *urlsData = [NSJSONSerialization dataWithJSONObject:urls options:0 error:nil];
            NSString *urlsJSON = urlsData ? [[NSString alloc] initWithData:urlsData encoding:NSUTF8StringEncoding] : @"[]";
            NSString *pageURL = [activeURL.absoluteString stringByReplacingOccurrencesOfString:@"\\" withString:@"\\\\"];
            pageURL = [pageURL stringByReplacingOccurrencesOfString:@"'" withString:@"\\'"];
            NSString *seed = [NSString stringWithFormat:@"(function(){var page='%@',observed=%@,blockFilters=%@;async function run(){try{if(!globalThis.chrome||!chrome.storage||!chrome.runtime)return;var trackerdb=await import(chrome.runtime.getURL('utils/trackerdb.js'));var reqmod=await import(chrome.runtime.getURL('utils/request.js'));if(trackerdb.setup&&trackerdb.setup.pending)await trackerdb.setup.pending;var ExtendedRequest=reqmod.default;var compiled=blockFilters.map(function(f){try{return new RegExp(f);}catch(e){return null;}}).filter(Boolean);function isBlocked(url){for(var i=0;i<compiled.length;i++){try{if(compiled[i].test(url))return true;}catch(e){}}return false;}var u=new URL(page);var host=u.hostname;var parts=host.split('.');var stats={domain:parts.slice(Math.max(0,parts.length-2)).join('.'),hostname:host,url:page,trackers:[],timestamp:Date.now()};var byId={};observed.forEach(function(url,i){try{var blocked=isBlocked(url);var request=ExtendedRequest.fromRequestDetails({url:url,originUrl:page,tabId:1,requestId:'pocb-'+i,timeStamp:Date.now()});request.blocked=blocked;var metadata=trackerdb.getMetadata(request);if(!metadata)return;var tracker=byId[metadata.id];if(!tracker){tracker=byId[metadata.id]=Object.assign({},metadata,{blocked:false,modified:false,requestsCount:0,requests:[]});stats.trackers.push(tracker);}tracker.requestsCount=(tracker.requestsCount||0)+1;tracker.blocked=tracker.blocked||blocked;if(tracker.requests.length<100)tracker.requests.unshift({id:request.requestId,url:url,blocked:blocked,modified:false});}catch(e){}});var order=['advertising','site_analytics','customer_interaction','essential','audio_video_player','comments','pornvertising','social_media','cdn','hosting','extensions','unknown','unidentified'];stats.trackers.sort(function(a,b){var ai=order.indexOf(a.category),bi=order.indexOf(b.category);return (ai<0?999:ai)-(bi<0?999:bi);});var ids=['1'];try{chrome.tabs&&chrome.tabs.query&&chrome.tabs.query({active:true,currentWindow:true},function(tabs){try{var tab=tabs&&tabs[0]||{};if(tab.id!=null)ids.push(String(tab.id));seed(ids);}catch(e){seed(ids);}});}catch(e){seed(ids);}function seed(keys){var area=chrome.storage.session||chrome.storage.local;if(!area)return;area.get(['tabStats:v1'],function(existing){try{var next=existing&&existing['tabStats:v1']||{};next.entries=next.entries||{};next.ttl=next.ttl||{};keys.forEach(function(id){next.entries[id]=stats;next.ttl[id]=Date.now()+86400000;});area.set({'tabStats:v1':next});}catch(e){console.error('pocb extension stats seed failed',e);}});}}catch(e){console.error('pocb extension stats seed failed',e);}}run();setTimeout(run,150);setTimeout(run,600);})();", pageURL, urlsJSON, blockedRulesJSON];
            [action.popupWebView evaluateJavaScript:seed completionHandler:^(id seedResult, NSError *seedError) {
                (void)seedResult;
                if (seedError) NSLog(@"pocb extension popup seed error: %@", seedError.localizedDescription);
            }];
        }];
    }
