...
- (BOOL)isLoadingCompleteForWebExtensionContext:(WKWebExtensionContext *)context { (void)context; return YES; }
...
- (WKWebExtensionWindowState)windowStateForWebExtensionContext:(WKWebExtensionContext *)context { (void)context; return WKWebExtensionWindowStateNormal; }
- (BOOL)isPrivateForWebExtensionContext:(WKWebExtensionContext *)context { (void)context; return NO; }
- (void)focusForWebExtensionContext:(WKWebExtensionContext *)context completionHandler:(void (^)(NSError *))completionHandler { (void)context; if (g_browserWindow) g_browserWindow->raise(); completionHandler(nil); }
@end
...
(function(){
  ...
  chrome.runtime = chrome.runtime || { id: 'pocb', getURL: function(path){ return path || ''; }, sendMessage: function(){ var cb = arguments[arguments.length - 1]; if (typeof cb === 'function') cb(undefined); } };
  chrome.storage = chrome.storage || {};
  chrome.storage.local = chrome.storage.local || {
    get: function(keys, cb){ var data = {}; try { data = JSON.parse(localStorage.getItem('__pocb_extension_storage') || '{}'); } catch(e) {} if (typeof cb === 'function') cb(data); },
    set: function(items, cb){ var data = {}; try { data = JSON.parse(localStorage.getItem('__pocb_extension_storage') || '{}'); } catch(e) {} Object.assign(data, items || {}); localStorage.setItem('__pocb_extension_storage', JSON.stringify(data)); if (typeof cb === 'function') cb(); },
    remove: function(keys, cb){ var data = {}; try { data = JSON.parse(localStorage.getItem('__pocb_extension_storage') || '{}'); } catch(e) {} (Array.isArray(keys) ? keys : [keys]).forEach(function(k){ delete data[k]; }); localStorage.setItem('__pocb_extension_storage', JSON.stringify(data)); if (typeof cb === 'function') cb(); }
  };
  function ok(script){
